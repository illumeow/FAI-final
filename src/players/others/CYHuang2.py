...
class CYhuang2:
    ...
    def __init__(self, player_idx: int, time_budget: float = None,
                 min_samples_per_card: int = 4, rollout_depth: int = 2):
        ...
        # Rollout depth. 1 = simulate only this round (the original Model 0).
        # 2 = simulate this round + next round, with our own next-round move
        #     chosen as the best response over our remaining hand.
        # Higher depths are a real gamble: ~10x more cost per sample for each
        # extra round of lookahead. We expose this as a kwarg primarily for
        # A/B comparison vs the depth-1 baseline.
        if rollout_depth not in (1, 2):
            raise ValueError(f"rollout_depth must be 1 or 2, got {rollout_depth}")
        ...
        while iter_idx < max_iters:
            ...
            try:
                if self.rollout_depth == 1:
                    # Depth-1: one card per opponent, score this round only.
                    # See note above: drawing n_opp distinct cards from the
                    # unseen pool is distributionally equivalent to sampling
                    # each opponent's full hand and then their played card.
                    opp_cards = rng.sample(unseen_list, n_opp)
                    points = self._simulate_round_points(card, opp_cards, rows)
                else:
                    # Depth-2: see _depth2_sample for the full recipe.
                    points = self._depth2_sample(
                        card, hand, card_idx, rows, unseen_list, n_opp, rng,
                    )
    ...
    def _simulate_round_points(my_card: int, opp_cards, rows) -> int:
        ...
        """
        pts, _ = MCSPlayer._simulate_round(my_card, opp_cards, rows,
                                           return_rows=False)
        return pts

    @staticmethod
    def _simulate_round(my_card: int, opp_cards, rows, return_rows: bool = False):
        """
        Generalization of `_simulate_round_points` that can also return the
        post-placement rows (needed for depth>=2 lookahead).

        If return_rows is False, the second tuple element is None.
        """
        ...
        for card, is_me in plays:
            ...
            if best_row_idx != -1:
                ...
                for r_idx in range(1, len(local_rows)):
                    key = (_row_score(local_rows[r_idx]),
                           len(local_rows[r_idx]), r_idx)
                    if key < best_key:
                        best_key = key
                        chosen = r_idx
                if is_me:
        ...
        return (my_points, local_rows if return_rows else None)

    def _depth2_sample(self, my_card, hand, card_idx, rows, unseen_list,
                       n_opp, rng):
        """
        Single depth-2 rollout. Returns the total points WE incur across
        this round + next round.

        Algorithm:
          1. Sample 3 opponent cards (this round) from unseen pool.
          2. Apply placement; record our round-1 points and resulting rows.
          3. Check if this is the last round of the game (hand had 1 card
             total). If so, return only round-1 points.
          4. For round 2, sample 3 different opponent cards from a depleted
             pool (unseen MINUS what they played round 1).
          5. We pick our round-2 move as the BEST RESPONSE — i.e. the card
             from our remaining hand that minimizes round-2 points given
             the round-2 opponent draws.
          6. Return round-1 points + round-2 points.

        Notes on correctness vs cost:
          * Best-response over our own next move is exactly the right thing
            to do for a "we play optimally" model. Without it, depth-2 just
            adds noise from a random next move and is strictly worse than
            depth-1.
          * The opponent next-round model is still uniform random (consistent
            with depth-1). We could try "best-response opponent" but that's
            both wrong (opponents don't know our hand) and expensive.
          * Cost per sample = O(hand_size_remaining) placement simulations,
            so worst case ~10 placements at start of game, ~2 at end.
        """
        # Step 1-2: round 1 simulation, capture resulting rows.
        opp_cards1 = rng.sample(unseen_list, n_opp)
        pts1, rows_after = MCSPlayer._simulate_round(
            my_card, opp_cards1, rows, return_rows=True,
        )

        # Step 3: are we out of cards? hand has len(hand) cards INCLUDING the
        # one we're testing; my "remaining hand for round 2" = hand minus my_card.
        # If that's empty, no round 2 to simulate.
        if len(hand) <= 1:
            return pts1

        # Step 4: build depleted pool for round-2 opponent draws.
        # Convert opp_cards1 to a set for O(1) lookup; pool is small (~80) so
        # rebuilding a list is fine and faster than fancier structures.
        played_set = set(opp_cards1)
        depleted_pool = [c for c in unseen_list if c not in played_set]

        # Edge case: if depleted_pool < n_opp (extreme late game), fall back
        # to depth-1 result.
        if len(depleted_pool) < n_opp:
            return pts1

        opp_cards2 = rng.sample(depleted_pool, n_opp)

        # Step 5: best-response round-2 move over our remaining hand.
        # Build my_remaining = hand without the card we played round 1.
        # Hand is sorted ascending; we iterate by index to skip card_idx.
        best_pts2 = None
        for j in range(len(hand)):
            if j == card_idx:
                continue
            my_next_card = hand[j]
            pts2, _ = MCSPlayer._simulate_round(
                my_next_card, opp_cards2, rows_after, return_rows=False,
            )
            if best_pts2 is None or pts2 < best_pts2:
                best_pts2 = pts2

        # best_pts2 is None only if hand had 0 remaining cards — guarded above.
        return pts1 + best_pts2
